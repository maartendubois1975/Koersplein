...
const instruments=(raw.shares||[]).map(x=>({isin:x.isin,name:x.name,company:x.name,symbol:x.symbol,ticker:x.symbol,mic:(x.mic||'XPAR'),market:(x.mic||'XPAR'),currency:'EUR',countryCode:'FR',provider:'yahoo-chart'}));
if(instruments.length<700)throw new Error(`Parijse catalogus onverwacht klein: ${instruments.length}`);
const markets=[{mic:'XPAR',code:'paris',name:'Euronext Paris',exchangeGroup:'Euronext',countryCode:'FR',currency:'EUR',timezone:'Europe/Paris'},{mic:'ALXP',code:'paris-growth',name:'Euronext Growth Paris',exchangeGroup:'Euronext',countryCode:'FR',currency:'EUR',timezone:'Europe/Paris'},{mic:'XMLI',code:'paris-access',name:'Euronext Access Paris',exchangeGroup:'Euronext',countryCode:'FR',currency:'EUR',timezone:'Europe/Paris'}];
