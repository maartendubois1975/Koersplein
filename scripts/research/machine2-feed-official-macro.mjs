...
const quarantine=[];const report={retrievedAt,requests:0,records:0,eligibleForTraining:0,quarantined:0,failures:[]};
for(const s of series){try{report.requests++;const r=await fetch(s.url,{headers:{Accept:'text/csv'}});if(!r.ok)throw new Error(`${r.status} ${r.statusText}`);const text=await r.text();for(const row of csvRows(text)){const date=row.TIME_PERIOD||row.TIME_PERIOD_START||row.TIME_PERIOD_END;const raw=row.OBS_VALUE;if(!date||raw===''||!Number.isFinite(Number(raw)))continue;const e={source:s.source,sourceTier:'A',urlOrStableId:s.url,observedAt:`${date}T00:00:00Z`,publishedAt:null,availableAt:null,entity:'EURO_AREA',family:s.family,signalName:s.signalName,valueOrClaim:Number(raw),revisionStatus:'CURRENT_OBSERVATION_NOT_PROVEN_VINTAGE',retrievedAt,trainingEligible:false,quarantineReason:'Historische publicatie-/beschikbaarheidstijd nog niet bewezen'};e.contentHash=crypto.createHash('sha256').update(JSON.stringify(e)).digest('hex');quarantine.push(e);}}catch(err){report.failures.push({signalName:s.signalName,error:String(err)});}}
report.records=quarantine.length;report.quarantined=quarantine.length;
await fs.writeFile(`${out}/official-macro-quarantine.jsonl`,quarantine.map(x=>JSON.stringify(x)).join('\n')+'\n');
